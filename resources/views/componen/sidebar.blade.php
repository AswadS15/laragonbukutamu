<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-16 transition-transform -translate-x-full   sm:translate-x-0 " aria-label="Sidebar">
    <div class="h-full ps pb-4 overflow-y-auto bg-emerald-700">
        ...
        <ul class="space-y-2 font-medium ps-3 pt-3">
            <li >
                <a href="/dashboard" class="{{ Str::startsWith(request()->path(), 'dashboard')  ? 'active' : '' }} {{ Str::startsWith(request()->path(), 'dashboard') ? '' : 'text-white' }} {{ Str::startsWith(request()->path(),'dashboard') ? '' : 'opacity-75' }} flex items-center p-2 rounded-s-lg hover:bg-white group transition-all hover:ms-5 hover:opacity-100 text-white/75">

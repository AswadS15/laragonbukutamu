<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-16 transition-transform -translate-x-full   sm:translate-x-0 " aria-label="Sidebar">
    <div class="h-full ps pb-4 overflow-y-auto ">
        ...
        <ul class="space-y-2 font-medium ps-3 pt-3">                
            <li class="">
                <a href="/dashboard" class="{{ Str::startsWith(request()->path(), 'dashboard') ? 'active' : '' }} {{ request()->is('dashboard') ? '' : 'text-white' }} {{ Str::startsWith(request()->path(),'dahsboard') ? '' : 'opacity-75' }} flex items-center p-2  hover:bg-white hover:ms-5 transition-all  group rounded-s-lg hover:opacity-100">
